...
import { useCarritoStore } from '@/stores/carritoStore'; // Importa el store

const carritoStore = useCarritoStore(); // Usa el store

onMounted(() => {
   carritoStore.obtenerProductos(); // Llama a la función del store
});

// Computed para obtener la cantidad total de productos
...
<template>
   ...
   <nav class="bg-teal-500 w-full">
      <div class="dark:bg-black grid grid-cols-4">
         ...
         <div class="col-span-1 flex justify-end gap-5 items-center p-5 relative"> <!-- Añadido 'relative' -->
            ...
            <button data-modal-target="default-modal" data-modal-toggle="default-modal" type="button">
               ...
               <span v-if="totalCantidad > 0" class="absolute top-4 -right-1 bg-red-600 text-white text-xs rounded-full px-1 transform translate-y-[-50%]">
                  {{ totalCantidad }}
               </span>
            </button>
         </div>
      ...
      <div class="w-full flex justify-center gap-5 items-center py-4">
         ...
         <Link :href="route('products')" class="hover:underline text-white">Productos</Link>
            ...
            <div class="p-4 md:p-5 space-y-4">
               <table class="min-w-full border-collapse border border-gray-200">
                ...
                <div class="mt-5 flex justify-center items-end">
                  <a :href="route('cart.index')" class="w-full flex justify-center items-center bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-4 rounded mb-2">Finalizar Compra</a >
